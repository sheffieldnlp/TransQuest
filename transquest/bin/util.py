...
def evaluate_model(test_sets, config, run, model=None):
    if model is None:
        model = QuestModel(
            config["model_type"], config["best_model_dir"], use_cuda=torch.cuda.is_available(), args=config
        )
    ...
    for lang_pair in sorted(test_sets):
        ...
        for r in range(config["n_fold"]):
            ...
            if config["regression"]:
                ...
                draw_scatterplot(
                    test_set.df, "labels", _preds_col(r), plot_path, config["model_type"] + " " + config["model_name"]
                )
